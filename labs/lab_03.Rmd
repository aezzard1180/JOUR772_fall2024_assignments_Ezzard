...
**A2**: The 2021 fiscal year had the most total amount. There is some inconsistency with the years mentioned. 2016, 2014, 2012 and 2011 are all missing, because of this, is 2021 really the fiscal year with the highest amount? If I were reporting on this I would call around to see if the data was just possibly not included in this dataset by mistake or if the amount really wasn't collected those years.
...
**A3**: Whiting turner contracting has the word company spelled out and also abbreviated to co. Storage Logic of Maryland Inc has the company written as StorageLogic and Storage Logic. Ayers Saint Gross has the word incorporated spelled out, the abbreviated version as inc and one doesn't have either included after the company name. To clean the data I would exclude the words and abbreviations of "company" and "incorporated". To change it I would use the distinct function and then probably use case_when if there are still issues. In the case of Storage Logic, I'm not sure what I would do to fix that, maybe just clean it up or find a way to make all company names with two words be spaced out.
...
**A4**: Stem cell research grants have gone down since 2009, but the amounts go up and down fairly steadily in the years after 2009, except for the grants between 2021 and 2022. The grant for 2021 may be way smaller because that's the year after the onset of the pandemic and a lot of the money that would be used for stem cell research was likely going to COVID treatments.

```{r}
standardized_grants_loans <- grants_loans |>
  filter(str_detect(description, "STEM CELL RESEARCH GRANT") | 
           str_detect(description, "Stem Cell Research Grant") | str_detect(description, "Stem Cell Reasearch Grant, Pre-Commercialization Research")) |>
  mutate(
    description_upper = str_to_upper(description))

standardized_grants_loans |>
