...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "YOUR KEY HERE")
```
...
Smith_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure, including headings, with no yapping: spec(title, year, role, director, co-stars, awards)", text),
  model = "llama-3.1-8b-instant"
)
...
**Answer:** Mostly all of the information was translated over. The information that was not translated were the other awards that she won because they were not associated with any movies that were mentioned in the article.
...
r_code_example_response <- axolotr::ask(
  prompt = paste("Given the following code, exaplain what it does to the UMD course)", r_code_example),
  model = "llama-3.1-8b-instant"
)
...
**Answer:** Yes, the answer is correct. At the end it even explained what this type of data analysis would be used for.

------------------------------------------------------------------------

## Q3. Helping with code debugging: paste the code block of an answer you had for a previous lab down here and ask Grok if that code is correct, based on the question in the lab. What do you think about its response? (Q4 Lab 2)
...
lab_question_response <- axolotr::ask(
  prompt = paste("Given the following question, is my answer correct?)", lab_question, your_r_code_lab),
  model = "llama-3.1-8b-instant"
)
...
**Answer:** I think Groq's response is fair. It said that I'm mostly correct, but gave me a more efficient way to answer the question. I like looking at different ways to solve something.
...
idea_response <- axolotr::ask(
  prompt = "I have a dataset of data on general inforamtion about collisions and deatials of all traffic collisions on county and local roadways in Montgomery County, as collected via the Automated Crash Reporting System (ACRS) of the Maryland State Police, and reported by the Montgomery County Police, Gaithersburg Police, Rockville Police, or the Maryland-National Capital Park Police. Suggest some news stories that I could porduce based on the data analyses",
  model = "llama-3.1-8b-instant"
)
...
**Answer:** Groq gave some pretty good suggestions. Number 5 was my favorite which mentioned looking at traffic collisions and comparing it to pedestrian safety. That was an interesting approach to the data.
...
#Add code below to talk to Groq and display its response
groq_response <- axolotr::ask(
  prompt = "Write a 500-word news artilce about a car crash at the intersection between KNox Road and Regents Drive in Colllege Park, Maryland",
  model = "llama-3.1-8b-instant"
)
...
**Answer:** One issue that I see are the names. It's fine that the witnesses and individuals involved are fake names, considering the story isn't real, but for officials, like the police chief of College Park, Groq used a fake name rather than the real police chief. The story isn't 500 words either. I transferred it into a word doc to check and the word count is 397. Even with the headline and photo caption, the whole response is only 439 words. Groq did follow AP style well. When I did ask Groq to generate the story I specified the roads as being in College Park.
